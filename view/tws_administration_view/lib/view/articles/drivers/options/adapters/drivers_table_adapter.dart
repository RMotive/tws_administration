...
final class _TableAdapter extends TWSArticleTableAdapter<Driver> {
  ...
  Widget _removeDialog(bool exceptionFlag, String xMessage, Driver set, BuildContext context, Future<void> Function() onAccept){
    ...
    return TWSConfirmationDialog(
      ...
      statement: Text.rich(
        ...
        exceptionFlag? TextSpan(
          ...
          text: 'Are you sure you want to delete this $entity: ${set.employeeNavigation?.identificationNavigation?.name} with license ${set.driverCommonNavigation?.license}?'
        ),
      ),
      onAccept:() async {
        await onAccept();
  ...
  @override
  Future<bool> onRemoveRequest(Driver set, void Function() closeReinvoke, BuildContext context) async {
            ...
            return !exceptionFlag? _removeDialog(
              ...
              () async {
                if(!clicked){
                  clicked = true;
                  String auth = _sessionStorage.session!.token;
                  MainResolver<Driver> resolver = await Sources.foundationSource.drivers.delete(set, auth);
